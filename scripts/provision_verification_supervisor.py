...
def _install_trusted_git_wrapper(
    ...
    content = (
        ...
        "    cursor = 2\n"
        "allowed = {\n"
        "    'cat-file', 'diff', 'grep', 'log', 'ls-files', 'ls-tree', 'merge-base',\n"
        ...
        "        patterns = 0\n"
        "        while (\n"
        "            cursor + 1 < len(options)\n"
        "            and options[cursor] == '-e'\n"
        "            and safe_path(options[cursor + 1])\n"
        "            and patterns < 64\n"
        "        ):\n"
        "            cursor += 2\n"
        "            patterns += 1\n"
        "        return (\n"
        "            patterns > 0\n"
        "            and len(options) == cursor + 2\n"
        "            and safe_ref(options[cursor])\n"
        "            and options[cursor + 1] == '--'\n"
        "        )\n"
        "    if command == 'log':\n"
        "        return options == [\n"
